...
//        }
    ...
    public void render(float delta) {
        ...
        if (!initialize && !playerTurn) {
            executeEnemyAttack(game.batch);
            spawnEnemies(game.batch);
            generateEnemySpawns(game.batch);
            ...
            if (playerList.isEmpty() || vitalList.isEmpty()) {
                ...
                enemyAttack(game.batch);
                playerTurn = true;
            }

        } else if (playerTurn) {
            playerAction(game.batch);
        }
    }
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                if(board[j][i] != null){
                    ...
                    game.batch.draw(board[j][i].getTexture(), centre.x + 50 - tileSize/2, centre.y + halfHeight - 16);

                    if(board[j][i] instanceof Damageable){
                        game.font.setColor(Color.BLUE);
                        game.font.draw(batch, Integer.toString(((Damageable) board[j][i]).getHealth()), centre.x + 50, centre.y + halfHeight - tileSize/4);
                    }
